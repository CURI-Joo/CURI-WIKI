...
export async function POST(request: NextRequest) {
  if (!isDemoMode()) {
    try {
      ...
      if (authError) {
        console.warn('[Telegram] Auth error:', authError.message);
      }

      if (!user) {
        console.warn('[Telegram] No authenticated user found, skipping auth check for notification');
        // Still allow the notification — the request already passed through middleware
        // and the issue was successfully created by the authenticated client
      }
    } catch (err) {
      console.warn('[Telegram] Auth check failed, proceeding with notification:', err);
    }
  }
